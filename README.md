...
### リポジトリのクローン
以下のコマンドを実行してください。  
`my-latex-project` はご自身の LaTeX プロジェクトのディレクトリ名に置き換えてください。  
...
独自のスタイルファイルを認識させるために `macros` フォルダを `texmf` ツリー内に配置します。

#### 1. 配置パスの確認
`texmf` ツリーの場所は OS や TeX 環境によって異なります。  
以下のコマンドを実行してご自身の環境のパス（TEXMFHOME）を確認してください。  
```bash
...
#### 2. ディレクトリの作成とファイルの配置
確認したパス (以下`<TEXMF>`) に合わせて以下の操作を行います。
...
```
**動作確認**  
以下のコマンドを実行してパスが表示されれば成功です。
```bash
kpsewhich sn-preamble.sty
```
### 3. 文献データベース (`.bib`) ファイルの配置
...
VSCode 全体の設定ファイルです。  
LaTeX を利用する際のエディタの挙動や LTeX の設定などを記述しています。  
確認して必要に応じて変更してください。  
特に LTeX の辞書設定 `ltex.dictionary` の `firstname` と `lastname` はご自身の名前に変更してください。  
...
リンクと相互参照の設定です。  
`hyperref` が未ロードであれば既定値のオプションで読みます。  
`bookmark` と `cleveref` もここでまとめて読みます。  
`hyperref` のオプションは後ろで `\hypersetup` で変更できます。  

### [`sn-locale.sty`](macros/sn-locale.sty)
`lang=ja` / `lang=en` のオプションを受け取ります。  
オプションを省略した場合は `sn-preamble.sty` と同様の自動判定を行います。  

### [`sn-bib.sty`](macros/sn-bib.sty)
`biblatex` のスタイルファイルです。  
`biblatex` を使うときはこれを読みます。  

### [`sn-code.sty`](macros/sn-code.sty)
コードスタイルのファイルです。  
`lstlisting` 環境のスタイルを定義しています。  
コードを表示するときはこれを読みます。  

## tex ファイルでの使用例

### 普段の文書で使うとき
[`latex.json.code-snippets`](.vscode/latex.json.code-snippets) に`snart`というスニペットを用意しています。  
`sn-preamble.sty` を読み込むだけでエンジンや言語の自動判定も含めて基本的なスタイルが適用されます。  
```tex
...
[`latex.json.code-snippets`](.vscode/latex.json.code-snippets) に`snart-mod`というスニペットを用意しています。  
`sn-preamble.sty` を使わず必要なものを順番に読みます。  
`sn-preamble.sty` の内容を参考にして以下のように書いてください。  

```tex
\documentclass[a4paper,dvipdfmx,11pt]{article}

% 自分で指定したいならsn-core より前に書く (省略可)
...
- `sn-preamble` 系のファイルは既存の `cls` や他の `sty` と競合しない範囲で使います。
